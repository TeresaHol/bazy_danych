...
--l
select avg(pensja.kwota::numeric), min(pensja.kwota), max(pensja.kwota) from pensja where pensja.stanowisko = 'seller';
--m
...
pracownicy.id_pracownika = wynagrodzenie.id_pracownika left join pensja on pensja.id_pensji = wynagrodzenie.id_pensji
 join premia on premia.id_premii = pensja.id_premii;
--6g
